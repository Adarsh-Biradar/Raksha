import {PaymentActions} from './PaymentScreen';
...
export function CreateTransaction({request,merchants}:{request:Request;merchants:string[]}){
 ...
 const fields=[['merchant','Merchant name','text'],['accountId','Customer account ID','text'],['amount','Amount (INR)','text'],['country','Country code','text'],['deviceId','Device ID','text'],['ipAddress','Customer IPv4 address (optional)','text'],['phoneNumber','Customer mobile number with country code','tel'],['failedAttempts','Preceding failed attempts','number']];
 ...
 <fieldset disabled={busy||Boolean(submitted)} className="transaction-fields">{fields.map(([key,label,type])=><label key={key}>{label}<input type={type} inputMode={key==='amount'?'decimal':undefined} list={key==='merchant'?'merchant-options':undefined} required={key!=='ipAddress'} maxLength={key==='merchant'?100:key==='country'?2:key==='phoneNumber'?40:key==='ipAddress'?45:80} pattern={['accountId','deviceId'].includes(key)?'[A-Za-z0-9_-]{1,80}':key==='country'?'[A-Za-z]{2}':undefined} min={key==='failedAttempts'?0:undefined} max={key==='failedAttempts'?100:undefined} step={key==='failedAttempts'?1:undefined} value={form[key as keyof typeof form]} onChange={e=>setForm({...form,[key]:e.target.value})}/></label>)}</fieldset>
