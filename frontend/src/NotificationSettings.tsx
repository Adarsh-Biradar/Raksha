...
export function NotificationSettings({request}:{request:Request}){
 ...
 const dirty=settings.enabled!==saved?.enabled||settings.minimum_classification!==saved?.minimum_classification||settings.recipients!==saved?.recipients||(settings.merchant_phone||'')!==(saved?.merchant_phone||'');
 ...
 <label>Merchant SMS number<input type="tel" maxLength={16} value={settings.merchant_phone||''} placeholder="+15555550100" onChange={e=>setSettings({...settings,merchant_phone:e.target.value})}/><small>E.164 format (e.g. +15555550100). Receives one SMS when a transaction succeeds and one when it fails. Leave blank to disable SMS alerts.</small></label>
 ...
 <p className="muted small">Sent to the merchant number on transaction success or failure. Failures retry up to three attempts.</p>
