...
public class EmailAlerts {
 ...
 void enqueue(UUID transactionId,String classification,int score){
  ...
  for(String recipient:recipients(settings.get("recipients")))
   db.update("INSERT INTO email_deliveries(id,alert_id,recipient,classification,score,transaction_id) VALUES(?,?,?,?,?,?) ON CONFLICT(alert_id,recipient) DO NOTHING",UUID.randomUUID(),alert,recipient,classification,score,transactionId);
 }
   ...
   try{
    Boolean found=tx.execute(status->{
     ...
     if(!allowed){
      ...
      fraud.audit("mail-worker","EMAIL_CANCELLED",id,"Notification policy no longer permits delivery");return true;
     }
     try{
      ...
      fraud.audit("mail-worker","EMAIL_SENT",id,"SMTP server accepted the message");
     }catch(Exception e){
      ...
      fraud.audit("mail-worker","EMAIL_ATTEMPT_FAILED",id,reason);
     }
     return true;
    });
