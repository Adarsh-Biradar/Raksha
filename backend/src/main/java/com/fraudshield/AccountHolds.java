...
public class AccountHolds {
 ...
 public boolean block(UUID id,String account,String actor,FraudService service){
  ...
  service.audit(actor,"TRANSACTION_BLOCKED",id,"account="+account+";case="+holds.get(0).get("alert_id"));return true;
 }
 public void place(UUID transaction,String account,FraudService service){
  UUID alert=db.queryForObject("SELECT id FROM alerts WHERE transaction_id=?",UUID.class,transaction);
  if(db.update("INSERT INTO account_holds(alert_id,account_id) VALUES(?,?) ON CONFLICT DO NOTHING",alert,account)==1) service.audit("risk-worker","ACCOUNT_HELD",account,"score=100;case="+alert);
 }
 public void release(UUID alert,String account,String actor,FraudService service){
  if(db.update("UPDATE account_holds SET released_at=now(),released_by=? WHERE alert_id=? AND released_at IS NULL",actor,alert)==1) service.audit(actor,"ACCOUNT_HOLD_RELEASED",account,"case="+alert+";remaining="+active(account).size());
 }
}
