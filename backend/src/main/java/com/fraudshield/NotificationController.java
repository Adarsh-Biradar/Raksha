...
public class NotificationController {
 ...
 @GetMapping("/settings") Map<String,Object> settings(Principal actor){
  Map<String,Object> result=new HashMap<>(db.queryForMap("SELECT * FROM notification_settings WHERE org_id=?",OrgUserDetails.of(actor)));
 ...
 @PutMapping("/settings") @Transactional Map<String,Object> save(@Valid @RequestBody Settings input,Principal actor){
  ...
  int changed=db.update("UPDATE notification_settings SET version=version+1,enabled=?,minimum_classification=?,recipients=?,merchant_phone=? WHERE org_id=? AND version=?",input.enabled(),input.minimumClassification(),String.join("\n",recipients),merchantPhone,org,input.version());
  if(changed==0)throw new ResponseStatusException(HttpStatus.CONFLICT,"Notification settings changed; reload and retry");
  fraud.audit(actor.getName(),"EMAIL_SETTINGS_UPDATED","1",fraud.encode(Map.of("enabled",input.enabled(),"minimumClassification",input.minimumClassification(),"recipientCount",recipients.size())),org);
  return settings(actor);
 }
 @GetMapping("/deliveries") List<Map<String,Object>> deliveries(Principal actor){return db.queryForList("SELECT * FROM email_deliveries WHERE org_id=? ORDER BY created_at DESC LIMIT 100",OrgUserDetails.of(actor));}
 @GetMapping("/sms-deliveries") List<Map<String,Object>> smsDeliveries(Principal actor){return db.queryForList("SELECT * FROM sms_deliveries WHERE org_id=? ORDER BY created_at DESC LIMIT 100",OrgUserDetails.of(actor));}
 @PostMapping("/test") @Transactional Map<String,Object> test(Principal actor){
  UUID org=OrgUserDetails.of(actor);
  Map<String,Object> settings=db.queryForMap("SELECT * FROM notification_settings WHERE org_id=? FOR UPDATE",org);
  ...
  if(db.queryForObject("SELECT count(*) FROM email_deliveries WHERE org_id=? AND classification='TEST' AND created_at>now()-interval '1 minute'",Long.class,org)>0)
   throw new ResponseStatusException(HttpStatus.TOO_MANY_REQUESTS,"Wait one minute before another test");
  List<UUID> ids=new ArrayList<>();
  for(String recipient:recipients){UUID id=UUID.randomUUID();ids.add(id);db.update("INSERT INTO email_deliveries(id,recipient,classification,org_id) VALUES(?,?,'TEST',?)",id,recipient,org);}
  fraud.audit(actor.getName(),"EMAIL_TEST_QUEUED","1","Recipients="+recipients.size(),org);
 ...
 @PostMapping("/deliveries/{id}/retry") @Transactional Map<String,Object> retry(@PathVariable UUID id,Principal actor){
  UUID org=OrgUserDetails.of(actor);
  Map<String,Object> settings=db.queryForMap("SELECT * FROM notification_settings WHERE org_id=? FOR SHARE",org);
  List<Map<String,Object>> rows=db.queryForList("SELECT * FROM email_deliveries WHERE id=? AND org_id=? FOR UPDATE",id,org);
  ...
  fraud.audit(actor.getName(),"EMAIL_RETRY_QUEUED",id,"Manual retry",org);
