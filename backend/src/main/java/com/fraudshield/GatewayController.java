...
public class GatewayController {
 ...
 @GetMapping List<Map<String,Object>> list(){return payments.list();}
 @GetMapping("/{id}") Map<String,Object> status(@PathVariable UUID id){return payments.status(id);}
 @PostMapping("/{id}/checkout") Map<String,Object> checkout(@PathVariable UUID id,Principal actor){return payments.checkout(id,actor.getName());}
 @PostMapping("/{id}/reconcile") Map<String,Object> reconcile(@PathVariable UUID id,Principal actor){return payments.reconcile(id,actor.getName());}
 ...
 @PostMapping("/{id}/confirm") Map<String,Object> confirm(@PathVariable UUID id,@Valid @RequestBody Confirmation c,Principal actor){return payments.confirm(id,c.paymentId(),c.signature(),actor.getName());}
