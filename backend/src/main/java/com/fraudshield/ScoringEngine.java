...
public class ScoringEngine {
 private final JdbcTemplate db; private final FraudService service; private final EmailAlerts email; private final MeterRegistry meterRegistry;
 private static final Logger log=LoggerFactory.getLogger(ScoringEngine.class);
 public ScoringEngine(JdbcTemplate db,FraudService service,EmailAlerts email,MeterRegistry meterRegistry){
  this.db=db;this.service=service;this.email=email;this.meterRegistry=meterRegistry;
 }
 void score(UUID id) {
  ...
  if("SCORED".equals(t.get("status"))) return;
  ...
  Map<String,Object> history=db.queryForMap("""
   ...
   FROM transactions WHERE account_id=? AND occurred_at<? AND received_at<=? AND currency=?
  ...
  String features=service.encode(Map.of("historyCount",n,"averageMinor",average,"amountRatio",ratio,"deviationScore",z,"fiveMinuteCount",recent,"method","rules + statistical anomaly detection","matchedRules",matched,"ruleSnapshot",rules));
