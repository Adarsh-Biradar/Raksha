...
public class RuleLabController {
 ...
 @GetMapping Map<String,Object> get(){return lab.configuration();}
 @PutMapping @Transactional Map<String,Object> save(@Valid @RequestBody Settings input,Principal actor){
  db.queryForMap("SELECT version FROM risk_policy WHERE id=1 FOR UPDATE");
  int changed=db.update("UPDATE rule_lab SET version=version+1,mode=?,repeat_count=?,window_minutes=?,points=? WHERE id=1 AND version=?",input.mode(),input.repeatCount(),input.windowMinutes(),input.points(),input.version());
  if(changed==0)throw new ResponseStatusException(HttpStatus.CONFLICT,"Rule Lab changed; reload before saving");
  db.update("UPDATE risk_policy SET version=version+1 WHERE id=1");
  fraud.audit(actor.getName(),"RULE_LAB_UPDATED","REPEATED_AMOUNT",fraud.encode(input));
  return get();
 }
 @PostMapping("/preview") @Transactional(readOnly=true,isolation=Isolation.REPEATABLE_READ,timeout=15)
 Map<String,Object> preview(@Valid @RequestBody Candidate candidate){
  Map<String,Object> policy=db.queryForMap("SELECT * FROM risk_policy WHERE id=1");
  List<Map<String,Object>> rows=db.queryForList("""
   ...
   FROM (SELECT * FROM transactions WHERE status='SCORED' AND received_at>=now()-interval '30 days'
   ...
   """,candidate.windowMinutes(),candidate.accountId().trim(),candidate.accountId().trim());
  ...
  result.put("samples",samples);result.put("candidate",candidate);result.put("policy",policy);result.put("labVersion",get().get("version"));
 ...
 @GetMapping("/observations") List<Map<String,Object>> observations(){return db.queryForList("""
  SELECT o.*,t.account_id,t.merchant,t.amount_minor FROM rule_lab_observations o
  JOIN transactions t ON t.id=o.transaction_id ORDER BY o.created_at DESC LIMIT 100
  """);}
}
